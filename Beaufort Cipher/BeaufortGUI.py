...
def beaufort_cipher(text, key, mode="Repeat"):
    if mode == "Autokey":
...
def beaufort_decrypt_autokey(ciphertext, key):
    key = key.lower()
    result = ''
    key_extended = key
    key_index = 0
...
def handle_encrypt():
    ...
    if not key.isalpha() or not key:
        ...
        return
    ciphertext = beaufort_cipher(plaintext, key, mode)
...
def handle_decrypt():
    ...
    if not key.isalpha() or not key:
        ...
        return
    if mode == "Autokey":
        plaintext = beaufort_decrypt_autokey(ciphertext, key)
    else:
        plaintext = beaufort_cipher(ciphertext, key, mode)
